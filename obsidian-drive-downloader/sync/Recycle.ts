	...
	async restoreRun(syncRunId: string): Promise<number> {
		...
		for (const { sidecar, dataPath } of sidecars.map((s) => ({ sidecar: s, dataPath: this.dataPathFor(s) }))) {
			...
			try {
				...
				if (exists) {
					const file = this.app.vault.getAbstractFileByPath(sidecar.originalPath);
					if (!(file instanceof TFile)) {
						throw new Error(`Vault index could not resolve restore destination: ${sidecar.originalPath}`);
					}
