...
import { DriveFileEntry, SyncPair } from "../types";
 ...
 * Options: flat view sorted by newest modified/created and delete the Drive copy
 * (to trash) after sync.
 */
export class DriveFilePickerModal extends Modal {
	...
	private async pull(entry: DriveFileEntry, btn: HTMLButtonElement, statusEl: HTMLElement): Promise<void> {
		...
		try {
			const result = await this.pullFile(entry, {
