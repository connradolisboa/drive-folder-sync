...
import { Automation, AutomationAction, AutomationOutput, AutomationRunReport, PeriodicNotesPaths, FileAutomationSettings } from "../types";
...
import { pathMatchesFolderRule } from "../companion/rules";

const LOG = "[FileAutomations/Automation]";
...
export interface RunForFileResult {
	/** True when a transcription action requested source deletion. */
	deleteRequested: boolean;
	/** True when the engine removed the source from the vault. */
...
export class AutomationEngine {
	constructor(
		private app: App,
		private settings: FileAutomationSettings,
		private manifest?: AutomationManifestStore,
	...
	updateManifest(manifest: AutomationManifestStore): void {
	...
	countMatchingFiles(automationId: string): number {
		...
		return this.app.vault.getFiles().filter(
			(file) => file.extension.toLowerCase() === "pdf" && this.matchesTrigger(automation, file.path)
		).length;
	}
	...
	async runForAllMatchingFiles(
		...
		for (const file of files) {
			if (!this.matchesTrigger(automation, file.path)) continue;
			matched++;
			const version = this.versionFor(file);
			...
			if (opts.dryRun) {
				if (willRun) {
					ran++;
					preview.push({ vaultPath: file.path, willRun: true });
				} else {
					skipped++;
					preview.push({ vaultPath: file.path, willRun: false, skipReason: "already ran for this source version" });
				}
				continue;
			}
			...
			if (!willRun) {
				skipped++;
				this.manifest?.recordAutomationRun(file.path, automation.id, {
			...
			console.log(`${LOG} runForAllMatchingFiles: running "${automation.name}" for "${file.path}"`);
			try {
				const output = await this.runAction(
					automation.action,
					file.path,
					this.manifest?.get(file.path)?.companionPath,
				...
				if (automation.action.deleteFileAfterTranscription && output?.transcriptionWritten) {
					await this.deleteSource(file.path);
				}
				ran++;
			} catch (e) {
				console.error(
					`${LOG} runForAllMatchingFiles: "${automation.name}" failed for "${file.path}":`,
					e
				);
				this.manifest?.recordAutomationRun(file.path, automation.id, {
					lastRunAt: new Date().toISOString(),
	...
	async runForFile(opts: RunForFileOptions): Promise<RunForFileResult> {
		...
		const version = sourceVersion ?? (source instanceof TFile ? this.versionFor(source) : "");
		...
		if (matching.length === 0) {
			return { deleteRequested: false, sourceDeleted: false, errors: 0, transcriptionWritten: false };
		}
		...
		for (const automation of matching) {
			...
			const shouldRun = this.shouldRunAutomation(automation.id, vaultPath, version, force);

			if (!shouldRun) {
				console.log(`${LOG} Skipping automation "${automation.name}" for "${vaultPath}" — already ran for this source version`);
				if (this.manifest) {
					this.manifest.recordAutomationRun(vaultPath, automation.id, {
						lastRunAt: new Date().toISOString(),
						sourceVersion: version,
						result: "skipped",
					});
			...
			try {
				const output = await this.runAction(automation.action, vaultPath, companionPath, sourceCreatedTime, transcription, report);
				if (output) {
					output.automationId = automation.id;
					report.byId[automation.id] = output;
					(report.byType[automation.action.type] ??= []).push(output);
				}
				if (output?.transcriptionWritten) transcriptionWritten = true;
		...
		if (deleteRequested && allowDeleteRequest) {
			sourceDeleted = await this.deleteSource(vaultPath);
		}
		await this.manifest?.save();

		return { deleteRequested, sourceDeleted, errors, transcriptionWritten };
	}
	...
	matchingAutomationIds(vaultPath: string): string[] {
		return this.settings.automations
			.filter((automation) => automation.enabled && this.matchesTrigger(automation, vaultPath))
			.map((automation) => automation.id);
	}

	hasDeleteAfterTranscription(vaultPath: string): boolean {
		return this.settings.automations.some((automation) =>
			automation.enabled &&
			this.matchesTrigger(automation, vaultPath) &&
			automation.action.deleteFileAfterTranscription === true
		);
	}
	...
	async runForVaultChange(vaultPath: string): Promise<RunForFileResult> {
		...
		return this.runForFile({
			...
			sourceVersion: this.versionFor(file),
			force: false,
			allowDeleteRequest: true,
		});
	}
	...
	private async deleteSource(vaultPath: string): Promise<boolean> {
		this.bus?.emit("source-delete-start", { vaultPath });
		let sourceDeleted = false;
		try {
			...
			this.bus?.emit("source-delete-complete", { vaultPath, sourceDeleted });
		}
	}
	 ...
	 *
	 * The vault path plus mtime:size source version is the decision key, including
	 * for files that were never downloaded by the companion plugin.
	 */
	async runForFileAdHoc(
		...
		const entry = this.manifest?.get(vaultPath);

		// Honor skip flags from companion frontmatter (when we have a known companion).
		let companionPath: string | null = entry?.companionPath ?? null;
		const sourceCreatedTime = new Date(file.stat.ctime).toISOString();
		const version = this.versionFor(file);
		...
		const shouldRun = this.shouldRunAutomation(automationId, vaultPath, version, opts.force === true);

		if (!shouldRun) {
			if (this.manifest && !opts.dryRun) {
				this.manifest.recordAutomationRun(vaultPath, automationId, {
					lastRunAt: new Date().toISOString(),
					sourceVersion: version,
					result: "skipped",
				});
			...
			return { ran: false, skippedReason: "already ran for this source version" };
		}
		...
		console.log(`${LOG} runForFileAdHoc: running "${automation.name}" for "${vaultPath}"`);
		try {
			const output = await this.runAction(automation.action, vaultPath, companionPath, sourceCreatedTime, undefined);
			this.emitRun(vaultPath, automation, "success");
			if (this.manifest) {
				this.manifest.recordAutomationRun(vaultPath, automationId, {
					lastRunAt: new Date().toISOString(),
			...
			if (automation.action.deleteFileAfterTranscription && output?.transcriptionWritten) {
				await this.deleteSource(vaultPath);
			}
			return { ran: true };
		} catch (e) {
	...
	private shouldRunAutomation(
		...
		const prior = this.manifest.getAutomationRun(vaultPath, automationId);
		if (!prior) return true;
		if (prior.result === "error") return true;
		if (prior.sourceVersion !== sourceVersion) return true;
		return false;
	}
	...
	private matchesTrigger(automation: Automation, vaultPath: string): boolean {
		...
		return pathMatchesFolderRule(
			vaultPath,
			resolvedTrigger,
			automation.triggerScope ?? "all",
			automation.excludedSubfolders ?? []
		);
	}

	private versionFor(file: TFile): string {
	...
	private async ensureCompanionNote(
		...
		const desired = `${dir}${stem}.md`;
		for (let index = 0; index < 10_000; index++) {
			const candidate = index === 0 ? desired : desired.replace(/\.md$/i, ` (${index + 1}).md`);
			...
			if (existing instanceof TFile) {
				...
				if (fm?.sourceVaultPath === vaultPath) return existing;
				continue;
			}
			const note = await this.ensureNote(candidate);
			if (!note) return null;
			await this.app.fileManager.processFrontMatter(note, (fm) => {
				fm.companion = `[[${vaultPath.split("/").pop() ?? vaultPath}]]`;
				fm["companion-of"] = `[[${vaultPath.replace(/\.[^.]+$/, "")}]]`;
				fm.sourceVaultPath = vaultPath;
			});
			const entry = this.manifest?.ensure(vaultPath);
			if (entry) entry.companionPath = candidate;
			return note;
		}
		throw new Error(`Unable to allocate companion path for "${vaultPath}"`);
	}
