...
import type FileAutomationsPlugin from "../main";
...
const LOG = "[FileAutomations/Transcribe]";

export function openTranscribePickerForFile(
	app: App,
	plugin: FileAutomationsPlugin,
	file: TFile,
...
export async function transcribeCurrentFile(plugin: FileAutomationsPlugin): Promise<void> {
	...
	if (!plugin.settings.geminiEnabled) {
		new Notice("AI transcription is not enabled. Enable it in File Automations settings.");
		return;
	}
	const provider = plugin.settings.transcriptionProvider ?? "gemini";
	if (provider === "mistral" && !plugin.settings.mistralApiKey) {
		new Notice("Mistral API key is not set. Add it in File Automations settings.");
		return;
	}
	if (provider === "gemini" && !plugin.settings.geminiApiKey) {
		new Notice("Gemini API key is not set. Add it in File Automations settings.");
		return;
	}
...
class DestinationPickerModal extends Modal {
	constructor(
		app: App,
		private plugin: FileAutomationsPlugin,
	...
	private async resolveCompanion(): Promise<TFile | null> {
		...
		const manifestEntry = plugin.manifestStore.get(pdfFile.path);
		if (manifestEntry) {
			if (manifestEntry.companionPath) {
				const existing = app.vault.getAbstractFileByPath(manifestEntry.companionPath);
				if (existing instanceof TFile && plugin.companionManager.ownsSource(existing, [pdfFile.path])) {
					return existing;
				}
				manifestEntry.companionPath = null;
				manifestEntry.companionMtime = undefined;
				await plugin.manifestStore.save();
			}
		}
		const rule = plugin.companionManager.findRule(pdfFile.path);
		if (rule) {
			try {
				const result = await plugin.companionManager.ensure(
					pdfFile,
					rule,
					null,
					undefined,
					undefined,
					(candidate) => {
						const owner = plugin.manifestStore.findByCompanionPath(candidate);
						return !!owner && owner[0] !== pdfFile.path;
					}
				);
				const entry = plugin.manifestStore.ensure(pdfFile.path);
				...
				new Notice(`Failed to create companion note: ${(error as Error).message}`);
				return null;
			}
		}
		...
		for (let index = 0; index < 10_000; index++) {
			const candidate = index === 0
				? companionPath
				: companionPath.replace(/\.md$/i, ` (${index + 1}).md`);
			const existing = app.vault.getAbstractFileByPath(candidate);
			if (existing instanceof TFile) {
				const sourcePath = app.metadataCache.getFileCache(existing)?.frontmatter?.sourceVaultPath;
				if (sourcePath === pdfFile.path) {
					const entry = plugin.manifestStore.ensure(pdfFile.path);
					entry.companionPath = existing.path;
					entry.companionMtime = existing.stat.mtime;
					await plugin.manifestStore.save();
					return existing;
				}
				continue;
			...
			try {
				await ensureFolder(app, candidate);
				const created = await app.vault.create(
					candidate,
					`---\ncompanion: "[[${pdfFile.name}]]"\nsourceVaultPath: "${pdfFile.path}"\n---\n\n# ${pdfFile.basename}\n\n`
				);
				const entry = plugin.manifestStore.ensure(pdfFile.path);
				entry.companionPath = candidate;
				entry.companionMtime = created.stat.mtime;
				await plugin.manifestStore.save();
				return created;
			} catch (e) {
		...
		new Notice("Could not allocate a companion note path.");
		return null;
	}
	...
	private async runTranscription(
		...
		try {
			...
			const manifestEntry = this.plugin.manifestStore.get(this.pdfFile.path);
			const ruleLabel = manifestEntry?.ruleId
				? (this.plugin.settings.companionRules.find((rule) => rule.id === manifestEntry.ruleId)?.label ?? "")
				: "";
			await writeTranscription(this.app, destFile, transcription, mode, this.pdfFile.name, template, this.pdfFile.path, ruleLabel);
	...
	private async recordTranscription(pdfBytes: ArrayBuffer, destFile: TFile): Promise<void> {
		...
		const info = await analyzePdf(pdfBytes);
		const manifestEntry = this.plugin.manifestStore.get(this.pdfFile.path);
		const sourceVersion = `${this.pdfFile.stat.mtime}:${this.pdfFile.stat.size}`;
		const isCompanion = manifestEntry?.companionPath === destFile.path;
		const isDaily = destFile.path.includes(moment().format("YYYY-MM-DD"));
		const destType = isCompanion ? "companion" : isDaily ? "daily" : "note";

		store.recordTranscription(this.pdfFile.path, info.hash, info.pageCount, sourceVersion, {
...
function resolveDailyNotePath(app: App, plugin: FileAutomationsPlugin): string {
